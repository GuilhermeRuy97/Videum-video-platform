...
issues:
  ...
  - id: UIG-1
    status: resolved
    summary: "Capability 'Signup, login, account confirmation and password recovery screens' covered by TD-01/TD-04 but no verb matches the umbrella bullet literally in inventory"
    resolved_by: deferred_capability
advisories:
  - id: MC-cross-1
    status: open
  ...
  - id: MC-cross-2
    status: open
    summary: "Phase 02 capability 'User signup with email and password' not claimed by any slice's covers_capabilities"
  - id: MC-cross-3
    status: open
    summary: "Phase 02 capability 'Automatic user channel creation from the email prefix' not claimed by any slice's covers_capabilities"
  - id: MC-cross-4
    status: open
    summary: "Phase 02 capability 'Account confirmation via email with activation link' not claimed by any slice's covers_capabilities"
  - id: MC-cross-5
    status: open
    summary: "Phase 02 capability 'User login and session control' not claimed by any slice's covers_capabilities"
  - id: MC-cross-6
    status: open
    summary: "Phase 02 capability 'Logout' not claimed by any slice's covers_capabilities"
  - id: MC-cross-7
    status: open
    summary: "Phase 02 capability 'Password recovery: request via email → token link → reset' not claimed by any slice's covers_capabilities"
  - id: MC-cross-8
    status: open
    summary: "Phase 02 capability 'Signup, login, account confirmation and password recovery screens' not claimed by any slice's covers_capabilities"
---
...
_None._ All 7 current-scope TDs are `Scope: Frontend`; `## UI Inventory` is populated (3 screens), so the Scope-Subsection orphan check does not fire. Every capability cited by an inventory verb (`Signup…`, `Login…`, `Password recovery…`) is present in scope.
...
_None._ The Decision #29 FE↔BE contract-sync strategy remains covered (MD-1, resolved). The two phase-level bullets without a covering TD ("Transactional email sending service", "Automatic user channel creation…") are owned by the sibling backend slice `phase-02-auth`, not by this frontend slice — surfaced as cross-slice advisories below, not as MD against this slice.
...
_None._ Capabilities without an inventory verb (`Account confirmation…`, `Logout`, the reset-password destination of `Password recovery…`, and the umbrella `Signup, login, account confirmation and password recovery screens`) are all already recorded in `## Non-UI / Deferred Capabilities` (Status: deferred) — UIG condition 3 fails for each.
...
- **MC-cross-2** — Phase 02 capability "User signup with email and password" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth-frontend/covers_capabilities` (and/or `phase-02-auth` if the backend signup endpoint is in scope); (b) leave unclaimed.
- **MC-cross-3** — Phase 02 capability "Automatic user channel creation from the email prefix" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth/covers_capabilities` (backend — `phase-02-auth/TD-10` nickname allowlist); (b) leave unclaimed.
- **MC-cross-4** — Phase 02 capability "Account confirmation via email with activation link" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in both slices (FE for landing, BE for token consumption); (b) leave unclaimed.
- **MC-cross-5** — Phase 02 capability "User login and session control" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in both slices; (b) leave unclaimed.
...
- **MC-cross-7** — Phase 02 capability "Password recovery: request via email → token link → reset" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in both slices; (b) leave unclaimed.
- **MC-cross-8** — Phase 02 capability "Signup, login, account confirmation and password recovery screens" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth-frontend/covers_capabilities` (FE-exclusive); (b) leave unclaimed.
...
- **OQ-8** _(resolved_by deferred_capability)_ — "Account confirmation via email with activation link" marked deferred. Row in `## Non-UI / Deferred Capabilities`.
...
- **UIG-1** _(resolved_by deferred_capability)_ — Capability "Signup, login, account confirmation and password recovery screens" marked **deferred** in `## Non-UI / Deferred Capabilities`. Rationale (user-provided verbatim): "the account confirmation screen will not be implemented in this current phase, it will be deferred" — the umbrella bullet's full coverage requires the confirmation and reset-password destination screens, both already in Non-UI/Deferred rows above; the 3 ship-this-phase screens (signup, login, forgot-password) remain covered by their own inventory verbs. The umbrella bullet itself is deferred to the phase that lands the two missing screens.
