...
Vitest runs in `environment: "node"` (default) and loads `mocks/setup.ts` via `setupFiles`; MSW starts with `onUnhandledRequest: "error"` — any unintercepted `fetch` fails the test with `"request unhandled"`. Layout of `mocks/` (per-domain handlers + barrel, factories, server).
