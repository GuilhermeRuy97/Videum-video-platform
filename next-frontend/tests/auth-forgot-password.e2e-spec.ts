...
test.describe("auth-forgot-password", () => {
  // 1. Request sending of email with password reset link

  test("1.1 forgot-password-success-inline", async ({ page }) => {
  ...
  test("1.2 forgot-password-anti-enumeration", async ({ page }) => {
    ...
    await expect(status).not.toContainText(
      /not (exist|found)|no account/i
    )
  })

  test("1.3 forgot-password-error-400-and-client-side-validation", async ({
    page,
  }) => {
    await page.goto("/forgot-password")
