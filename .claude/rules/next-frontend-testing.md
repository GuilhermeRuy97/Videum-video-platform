...
**DOM rendering opt-in:** the default `node` environment does not render components/pages. Tests that render JSX/TSX (components, pages — rows in the "Test type selection" table) must opt into DOM rendering **per file** with the docblock `// @vitest-environment jsdom` at the top of the file (`jsdom` and `@testing-library/react` are already installed). Without this docblock, only pure logic/hooks/utils run under `node`.