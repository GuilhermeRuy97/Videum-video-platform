...
3. In **Step 0.5 (UI scope detection)**:
   - Phase mode: UI signal = any bullet in `covers_capabilities` (or phase bullets, fallback) matches UI phrasing (`Screen`, `Page`, `Area`, `Login`, `UI`) OR explicit frontmatter `ui_scope: true` (future extension point; ignore if absent).
...
1. Add `covers_capabilities` to `technical-decisions-auth-backend.md` frontmatter:
   ...
   covers_capabilities:
     ...
     - "Automatic user channel creation from email prefix"
