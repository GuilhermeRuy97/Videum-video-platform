...
The user may also request **continuous mode** at the start of the session with phrases like "execute everything", "don't pause between SIs", "run all at once", "autopilot". The default mode pauses between SIs for confirmation.
...
- **Mode detection (`PLAN_MODE`)**: read the plan frontmatter and decide whether the plan was built with `test_specs_aware: true` (modern) or without it (legacy). Modern plans are subject to the Test Specs preflight + JIT spec read documented below. Legacy plans skip both.
  ...
  - `PLAN_MODE=legacy` → old plans OR plans where the `test_specs_aware` frontmatter is absent. Skip the **Test Specs preflight** and the **Step 3 JIT extension** below. Keep current behavior: read Tests entries from the SI, write test code from one-sentence prose (status quo). Note: `phase-a.md` emit rules never write `test_specs_aware: false` by construction — the grep `grep -c "^test_specs_aware: true$"` would return 0 in that case and fall into legacy anyway; but the explicit "false" state is not a legitimate builder output.
  - `PLAN_MODE=modern` → new plans with `test_specs_aware: true`. Apply **Test Specs preflight** + **Step 3 JIT extension** below — even if the plan has no SI with `**Test Specs:**` (pure backend phase case: passes through the new preflight but all greps return empty = OK).

- **Test Specs preflight (only when `PLAN_MODE=modern`)**: detects MISSING and `_pending_` markers (hard abort) and STALE specs (soft warn + confirm). Three distinct conditions:
  - `_pending_` in plan → **abort** (plan-test-specs never ran)
  - spec MISSING → **abort** (spec was never created)
  - spec STALE ≤ 10 min → **ignore** (grace window: plan was regenerated shortly after the specs — common false positive)
  - spec STALE > 10 min → **warn + ask the user** (spec may be outdated; user decides whether to proceed or re-run /plan-test-specs)

  ```bash
  PLAN_MTIME=$(stat -c %Y "$PLAN")
  GRACE_SECONDS=600  # 10-minute grace window — covers plans re-generated shortly after /plan-test-specs

  # (a) Hard abort if _pending placeholders still remain (means /plan-test-specs never ran)
  PENDING=$(grep -c "^\*\*Test Specs:\*\* _pending" "$PLAN" 2>/dev/null)
  [ "${PENDING:-0}" -gt 0 ] && echo "PENDING TEST SPECS: $PENDING SI(s) still have placeholder. Run /plan-test-specs <slug> first."

  # (b) MISSING = hard abort; STALE within grace window = ignore; real STALE = warn (do not abort)
  ...
  After running the script: if only `STALE` lines are present (no `PENDING` or `MISSING`), **do not abort automatically** — present to the user with context ("spec exists but is older than the plan by Xs; content may be outdated") and ask whether to proceed or re-run `/plan-test-specs`. Only `PENDING` and `MISSING` justify abort without user confirmation.
...
**3. Invoke `figma:figma-implement-design` with the narrow handoff.** Same handoff shape as SI-Xa (Decision #31 — no auth, no errors, no validation):
...
- `aligned` → byte-level match between DS value and Figma value (same Tailwind utility, custom-property reference, or mathematical equivalence)
- `minor drift` → deviation within DS token system, same token family/category (e.g., `rounded-xl` vs `rounded-md`)
- `relevant drift` → missing surface (variant / prop / state demanded but undeclared), system breakage (different token family), hardcoded leak (hex/rgb literal), or component file absent on disk
- `missing component` → file listed in Reused DS list does not exist on disk

For `minor drift` / `relevant drift`, compose `auto-Edit "<specifics>"` per the **3-form constraint** in schema § "Naming constraint for `auto-Edit` `<specifics>`" — try in order; first match wins:
...
- If present AND current Figma demand reproduces prior outcome → `status: aligned`, `decision: skip`, `Prior: "<prior summary> at <source_si> honored"`.
- If present AND current Figma demand DIVERGES from prior outcome → `status: relevant drift`; verb is `CONFLICT` rendered in the vertical hierarchical format per schema § Decision:
...
**6. Pause for user review at Step 6 (default mode).** The user opens `frontend-drift-report.md`, optionally edits Decisions on per-component H3 sections (the file-based override mechanism — see schema § File-based user override mechanism), saves, and responds "Proceed". `/implement` then proceeds to SI-NN.Xa, which reads the (potentially user-edited) report and applies decisions verbatim.
...
**3. Visual shell generation — invoke `figma:figma-implement-design` with the narrow handoff (Decision #31).** Pass:
...
**SI-Xb — Logic & wiring (≤5 Technical actions):** executed by `/implement` main thread OR framework-specific sub-skills when available. Actions map UI Contract fields → concrete code edits:

- Route guard (Auth requirement)
- Rendering strategy (TD-derived or placeholder)
- Endpoint wiring (using shared types per Decision #29)
...
**Conditional skip of SI-Xb (Decision #33):** some screens have no SI-Xb emitted (`/plan-build` decided pure-presentational). `/implement` simply doesn't see SI-Xb for those screens — SI-Xa alone completes the screen. No special handling needed.
...
Violating this stop is the single most common failure mode of this skill. Treat "Proceed to {next-SI-id}?" as a terminator, not a rhetorical question.
...
- **`Drift Report:`** drift audit-SI-specific extension (`SI-NN.X.0` only) — line summarizing the report's Quick scan tally for this screen in the form `<N> components (<a> aligned, <b> minor drift, <c> relevant drift, <d> missing)`. The numeric breakdown helps the user spot unusual outputs (e.g., `0 aligned` suggests something is off) before opening the file. Omit on every non-audit SI.
...
```
SI-02.7.0 done.
  Drift Report: 12 components (4 aligned, 5 minor drift, 3 relevant drift, 0 missing).
  Created: frontend-drift-report.md.
  Tests: no tests (audit-only). Proceed to SI-02.7a?

SI-02.8.0 done.
  Drift Report: 9 components (6 aligned, 1 minor drift, 1 relevant drift, 1 missing).
  Updated: frontend-drift-report.md.
  Tests: no tests (audit-only). Proceed to SI-02.8a?
...
SI-02.7a done.
  ...
  Tests: no tests (visual shell). Proceed to SI-02.7b?
...
SI-02.3 done.
  ...
  Tests: 8 passing. Proceed to SI-02.4?

SI-02.6 done.
  Created: components/auth/auth-provider.tsx
  Updated: app/layout.tsx
  Tests: 4 passing. Proceed to SI-02.7a?
```
...
- After each SI in default mode, **STOP** and wait for user approval before advancing — per-SI transitions via "Proceed to {next-SI-id}?" (phase mode: `SI-NN.X+1`; task mode: `SI-N+1`), and after the final SI via "Run final verification?" before starting the plan-level checks. Continuous mode applies only when the user explicitly requested it at the start of the session, in which case both pauses are skipped.
...
- **Narrow handoff to `figma:figma-implement-design`** (Decision #31). Pass only Figma URL, Reused DS, Server-connected component names, target paths. **Never** pass auth/rendering/error/validation info — those belong to SI-Xb.
- **SI-Xa and SI-Xb are distinct SIs, not sub-SIs of the same entity** (Decision #32). Each has its own Dependency Map entry, its own Tests table, its own Acceptance criteria, its own progress.md line. Completing SI-Xa marks Xa done; completing SI-Xb independently marks Xb done.
...
- **Screen SIs are Xa/Xb pairs OR SI-Xa alone** (Decision #33). Never emit SI-Xb without SI-Xa; never emit a bare `SI-NN.X` without letter suffix for screens (letter suffix is load-bearing).