...
description: "Generate a screen inventory for a project phase or task. Infers which screens need to be built or updated from the phase's capabilities (phase mode) or from the task's scope prose (task mode), asks the user for the corresponding Figma URLs, then extracts each screen's components via the Figma MCP, classifies them by behavior (Presentational / Local-interactive / Server-connected), and maps server-connected components to capabilities or scope. Use whenever the user asks to inventory screens, extract components from Figma for planning, or prepare the front-end inputs before /plan-context — including variations like 'inventory the screens', 'screen inventory for phase NN', 'screen inventory NN', 'screen inventory <task-slug>', 'extract components from Figma', or any mention of connecting Figma screens to planning before plan-context runs. Accepts a phase number (integer) OR a task slug (string), optionally with an upfront list of Figma URLs."
...
This skill is the front-end counterpart of `research`. It produces **verbs of intent** (e.g., "trigger new video upload"), not HTTP specifications — which lets `plan-validate` reconcile the UI and API sources via the UIG-N check and catch gaps in either direction.
...
1. **Scope source — mode-dependent:**
   - **Phase mode** (slice slug, or integer `NN` resolving to one UI-bearing slice): first bounded-read the slice's `docs/decisions/technical-decisions-{slug}.md` frontmatter.
     ...
     - If `covers_capabilities` is omitted (monolithic phase) → fall back to `docs/project-plan.md` → `### Phase NN — ...` section. Capabilities there are the source of truth; every verb maps verbatim to a phase bullet.
...
|---|------------------------------|----------------------|--------------|
| 1 | Signup screen                | ABC123:1234:5        | completed    |
| 2 | Login screen                 | ABC123:1234:10       | in_progress  |
| 3 | Account confirmation screen  | ABC123:1234:20       | pending      |
...
- ~~Confirmation error screen~~ — user: "it's just a toast, there's no screen"

## Decisions log

- [DECISION: VideoCard fetcha sozinho ou recebe do pai?] — pending
- ✓ [DECISION: Is SortDropdown local or server?] — resolved: local (applied to screen 2)
```
...
- **Phase exists in `docs/project-plan.md`** (the `### Phase NN — …` section matching the phase number). If not, stop.
- **Look for an existing progress file.** If found:
  ...
  - Resume from the first screen whose status is `pending` or `in_progress`, and tell the user: `"Found progress file with X/Y screens completed. Resuming from: <screen name>."`
...
**How:** pick any `fileKey:nodeId` from the reconciled screen list → call `mcp__plugin_figma_figma__get_variable_defs` → compare returned tokens against the matching blocks in `globals.css` (`--color-*` → `@theme inline`; `--radius-*`, `--spacing-*`, and semantic theme tokens → `:root` / `.dark`). If drift is found, warn: `"Detected N tokens with drift between Figma and globals.css. Recommend running 'figma-audit-tokens' before inventorying. Proceed anyway?"`. If no drift, stay silent.
...
- **Explicit screen mentions.** Phrases like "Screen for …", "Page for …", "Panel for …", "Area for …", "Home page" are unambiguous screen references. Create one candidate per mention. Example: `"Signup, login, account confirmation, and password recovery screens"` → four candidates.
- **Implicit user-facing actions.** Capabilities that describe an action a user performs but do not name a screen (e.g., "Edit video information", "Draft → publish flow") usually require a screen to host the action. Create a candidate and mark it `[inferred]` so the user can confirm. These are the most error-prone — err on the side of listing them and letting the user remove what is not actually new UI.
- **Capabilities that cover multiple screens.** A single bullet may imply more than one screen. "Password recovery: request via email → token link → reset" implies at least two screens (request reset, reset password). Split them.

**What does NOT imply a screen:**

- **Backend services and infrastructure.** "Transactional email sending service", "Background processing service (queues)", "File storage service", "Database", "Queue" — these are back-end concerns; no screen.
- **Processing and automation.** "Automatic video processing", "Automatic thumbnail generation", "Automatic channel creation from email prefix" — happen without UI; no screen.
- **Cross-cutting concerns.** "Responsive layout", "Main flow tests", "Production environment and deploy" — not screens.
- **Components, not screens.** "Header/navbar with logo, search bar, login/avatar button" describes a component that lives across screens. It is **not** a candidate screen — it belongs to the inventory of whichever screens render it. Flag these separately as shared components, not as candidates.
...
- `[DECISION: "Password recovery: request → link → reset" — 2 screens or 3? Does the email link land on an intermediate page or redirect straight to the reset form?]`
- `[DECISION: "Signup and login screens" — two separate routes or one auth page with tabs?]`
- `[DECISION: "Video processing tracking" — does this imply a progress screen, or is it a notification/toast on the upload screen?]`
...
2. **Confirmation or removal.** The user may tell you a candidate is out of scope (e.g., "account confirmation is just a link, it has no own screen — the user lands directly on login"). Remove it from the final list.
...
2. If context.md exists but dir is incomplete (edge case — corruption or re-entrancy), apply slug collision check (Decision #22 of `unify-plan-and-implement.md`); create missing files via `mkdir -p` (idempotent, defensive).
3. Proceed with writes of inventory.md + inventory.progress.md.

Rationale: narrow co-ownership (per plan `Decision #18`) — screen-inventory creates only its own files, but does not substitute plan-context in defining task scope. Canonical workflow remains plan-context → screen-inventory.
...
3. **Build the Open questions section.** Collect, in order: residual decision dependencies from each screen's Observations subsection, any `[DECISION: ...]` markers still pending in the progress file's Decisions log block, and anything the parent surfaced during consolidation or validation that needs pipeline-level resolution (ingested by `plan-validate` as OQ-N). If there are none, omit the section entirely (remove the empty heading from the inventory file). **In extension runs**, the old Open questions section may contain narrative prose (e.g., "only Screen X and Screen Y" screen counts, or capabilities described as fully out-of-scope that are now partially covered) that contradicts the new state. Update that prose in place with `Edit` **before** replacing the bullet list from scratch.
...
- **Framework navigation note:** framework-provided client-side navigation links that just change route count as local-interactive unless the navigation itself is the server action (rare). E.g., Next.js `<Link>`, React Router `<Link>`, Vue Router `<router-link>`, SvelteKit `<a>` with prefetch.
...
**Good:** "Display paginated list of channel videos", "Trigger new video upload".

**Bad:** "POST /videos with multipart/form-data" (API contract — `/plan-build`'s job in the final artifact), "Fetch data" (too vague — data for what, for what purpose?).
...
A single component can have multiple verbs (e.g., a list item with both "display video" and "remove video" via a kebab menu). Record all of them — one row per verb in the verbs table.
...
- For any sentence that mentions "out of scope", "pending", "not inventoried", or references a specific screen by name — check whether the newly added screen changes that state. If so, `Edit` the Observation to reflect the new context.
- Patterns to detect: "flow X is out of scope for this phase", "destination route will exist as a stub", "none of the N screens was inventoried", "[link] points to a route that does not exist yet".
...
|--------------------------------------------|----------------------------------|----------------------------------|
| Display paginated list of channel videos | VideoCard                        | "Users view their videos"        |
| Trigger new video upload                 | UploadButton                     | "Users can upload videos"        |
| Remove user video                        | DeleteAction in VideoCard kebab  | "Users can delete own videos"    |
