...
| Plan file | progress.md | Mode | Behavior |
|---|---|---|---|
| exists | exists | append (auto) | Phase C — flow below |
| exists | absent | append (auto) | **Aborts**: `"progress.md absent for {scope}. Plan is built but /implement never ran (progress.md is created by /implement on first SI). Run /implement {scope} first, or pass --rebuild to rebuild the plan from scratch."` |
| absent | absent | full (auto) | Phases A+B (case 1). `progress.md` will be created by `/implement` on the first execution of the generated plan — not by `/plan-build`. |
| absent | exists | (degenerate) | **Aborts**: `"progress.md present without corresponding plan file in {scope}. Unexpected state — inspect manually; move/delete progress.md or recover the plan file before running."` |
...
1. Bounded grep `Grep -n '^### Frontend Runtime$' {target_path}` to locate the subsection. Two cases:
   - **Subsection present:** bounded read of the subsection range (start to next `^### `) → look for `#### {td-slug}/TD-NN —` heading. If present, the affected TD is already materialized; revisions edit the subsection in place (Pattern / Setup / Migration rows / Verification fields per the Revisions one-liner's prose). If absent (i.e., subsection exists for OTHER FE Runtime TDs but not this one), append a new `#### {td-slug}/TD-NN — {topic}` block via the `templates/tech-specs/frontend-runtime.md` template before processing SI-level deltas.
...
**Conditional `**Test Specs:** _pending /plan-test-specs_` placeholder on new SIs (modern plans only).** When plan-build emits a new SI (amendment OR new behavior) in modern mode (frontmatter declares `test_specs_aware: true`), apply the same field-based criterion described in `phase-b.md` § "Conditional emit of `**Test Specs:**` field":

- **Frontend wiring (Xb)** — emitted SI_ID ends with suffix `b` (e.g., `SI-NN.5b`) → emit `**Test Specs:** _pending /plan-test-specs_` in the header. This case occurs when C2 derives the new SI from a delta in `### UI Contracts` or inventory, and uses the `screen-si.md` Xb template.
- **Backend controller wiring** — emitted SI includes `**Route:** <METHOD> /...` in the header (regex `^\\*\\*Route:\\*\\* (GET|POST|PUT|PATCH|DELETE) /` on the rendered SI block) → emit placeholder.
- **Cross-layer SI** — title literally contains the `(cross-layer)` keyword (e.g., `### SI-NN.8 — Upload flow (cross-layer)`) → emit placeholder.

Plain backend service SIs, schemas, providers, frontend runtime SIs (via `templates/frontend-runtime-si.md`) DO NOT receive the placeholder — existing behavior preserved. The Frontend Runtime delta path (above) explicitly DOES NOT receive the placeholder because its SIs have no `**Route:**` and no `**Figma:**` by construction.

Mode detection is via frontmatter grep:

```bash
TEST_SPECS_AWARE=$(awk '/^---$/{f=!f;next} f' "$PLAN" | grep -c "^test_specs_aware: true$" || echo 0)
```

Modern (`TEST_SPECS_AWARE > 0`) → apply the rule above. Legacy (`TEST_SPECS_AWARE = 0`) → skip; new SIs are emitted without the `**Test Specs:**` field (back-compat — pre-migration behavior).
...
```
Append-mode detected {N} deltas. Proposed changes:

  1. SI-X.Y (pending) — Surgical Edit: replace `<old>` with `<new>` on line NN.
  2. SI-Z.W (completed) — Append SI-13 (amendment of SI-Z.W) — <one-liner>.
  3. (new behavior) — Append SI-14 — <delta-derived title>.

[Apply all] [Apply selected] [Cancel all]
```

The `AskUserQuestion` is single-select with 3 options. `[Apply selected]` triggers a follow-up multi-select where the user can opt-in per change. `[Cancel all]` aborts without writing.

## C4. Apply edits

For each accepted change, Edit the plan file:

- **Surgical Edit:** standard `Edit` call with `old_string` / `new_string`. The match is bounded to the SI's line range from C2.
...
**Modern-mode interaction with specs.** The `sources_mtime` refresh makes `mtime(plan) > Last sync(spec)` for every existing PRESERVED spec — which would trigger a false STALE in the `/implement` preflight. For this reason, in modern mode (frontmatter declares `test_specs_aware: true`), after `/plan-build` append-mode finishes, the user **must** run `/plan-test-specs <slug>`, which re-stamps `Last sync` of PRESERVED specs + processes NEW ones (placeholders added in C2). The conditional C7 hint (see `SKILL.md` § "Output contract → Phase C7") emits this instruction automatically when applicable; phase-c.md does NOT duplicate the output template — the conditional emit lives **exclusively** in `SKILL.md`.
...
- Each delta event processed and the action taken (Surgical Edit / Append amendment / Append new behavior / Annotate superseded).
