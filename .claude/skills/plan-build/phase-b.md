...
This file is loaded by `.claude/skills/plan-build/SKILL.md` after Gate 10 dispatches a resume (case 2: both required sentinels present), OR after Phase A's A5 pause when the user picks "Continue now". Read SKILL.md for preflight gates, dispatcher rules, hard rules, output contract, and rerun semantics — they apply to every phase.
...
- **New-session resume** — Gate 10 detected the sentinels; the model entered Phase B fresh with no Phase A working memory. **B1(a) and B1(b) are both mandatory.**
...
(b) **`## Technical Specifications` from `{target_path}`** — bounded read of the section Phase A wrote. Locate via `Grep -n '^## Technical Specifications$\|^## Dependency Map$' {target_path}` to get the start and end line numbers (Dep Map heading always sits immediately after Tech Specs, even when its body is still the sentinel). `Read` the line range between them. This loads the materialized Data Model entities, API Contracts endpoints + request/response shapes, Authorization Matrix rows, Error Catalog `errorCode`s, Events/Messages payloads, and (when present) UI Contracts per screen + Traceability Matrix.
...
**SI decomposition augmented for UI scope (Decisions #32/#33):**
...
- **Frontend screen SIs** — by default **split into 3 SIs per screen**: `SI-NN.X.0` (drift audit, 1 action — emits `frontend-drift-report.md` per the schema at `.claude/skills/plan-build/references/frontend-drift-report-schema.md`) + `SI-NN.Xa` (visual shell, ≤2 actions: apply drift decisions + figma plugin invocation, per Decision #32) + `SI-NN.Xb` (logic & wiring, ≤5 actions). Pure presentational screens skip `SI-NN.Xb` per Decision #33 criteria but still receive `SI-NN.X.0` + `SI-NN.Xa` (the audit precedes every visual shell — see "Conditional skip of SI-Xb" inside `templates/screen-si.md`).
- **Frontend Runtime SIs** — flat `SI-NN.X` (no letter suffix, no dotted suffix), one per `#### {td-slug}/TD-NN — {topic}` subsection in `### Frontend Runtime`. Each subsection produces ≥1 SI: 1 Setup SI (provider/root config/lib install) + N Migration SIs (1 per row in the subsection's Migration table) + 1 optional Verification SI (when Verification requires testing not covered by Setup/Migration). Discriminator vs backend SIs: the SI's Technical actions reference the `### Frontend Runtime → ####` subsection, NOT the backend subsections. Both backend SIs and Frontend Runtime SIs use plain `SI-NN.X` shape (no letter suffix). The conditional-skip rule for `SI-Xb` (Decision #33) does NOT apply to Frontend Runtime SIs — they are always plain SIs by construction. Read `.claude/skills/plan-build/templates/frontend-runtime-si.md` for the Setup/Migration/Verification template + canonical Dependency contract.
- **Cross-layer SIs** — 1 per end-to-end flow where backend and frontend must be implemented together (rare — usually prefer split backend SI + dependent SI-Xb). Title example: `SI-NN.8 — Upload flow (cross-layer)`.

**Numbering convention (CRITICAL — Decision #34 Round 7 fix):**
...
| `SI-NN.Xa` / `SI-NN.Xb` | FE screen split (Decision #32) | `SI-03.5a` (visual), `SI-03.5b` (logic) |
...
| `SI-NN.X.k` (k ≥ 1) | BE auto-split (Decision #34) | `SI-03.2.1` (Infra), `SI-03.2.2` (behavior) |
...
The partial artifact (scaffold + Tech Specs + sentinels) is left in place — Gate 10 on the next run will resume from B1 once the cache is fixed.
...
- Notes empty / pure-presentational ("wordmark + icon", "see screen: ...") → `custom-business-simple`.
...
**Description:** Install shadcn primitives via CLI registry; commit generated files in `components/ui/`.

**Technical actions:**

1. Run `npx shadcn@latest add <name1> <name2> ...` (alphabetical) — generates `components/ui/<name>.tsx` per primitive.
2. Commit `components/ui/<name>.tsx` per installed primitive.
...
**Description:** Unit tests for shadcn primitives installed in SI-NN.0.1 — variants, a11y, data-slot, event handlers.
...
**Description:** Author `<path>` per UI Contract — primitive under `components/ui/` not available in the shadcn registry.
...
**Description:** Author business components without state/scoring — pure presentational.
...
D-complex — 1 SI each (signal-keyword Notes):

```markdown
### SI-NN.0.k — Custom-business complex: <name>

**Description:** Author `<path>` — business component with {state | scoring | computed logic} per Notes "{notes verbatim}".
...
- **Audit-SI (`SI-NN.X.0`)** reads DS files from disk to perform value-level diff. If bootstrap hasn't run, files are missing and the audit produces false `missing component` rows. Bootstrap MUST precede the audit-SI.
...
## B4. Append SIs one at a time (no lookback) — replace SIs sentinel
...
For the **first** SI, `Edit` the SIs sentinel block:
...
## B5. Build Dependency Map — replace Dep Map sentinel
...
`Edit` the Dep Map sentinel block:
...
## B6. Build Deliverables — replace Deliverables sentinel
...
`Edit` the Deliverables sentinel block per the same Edit protocol.
...
Follow the shape below literally — English prose, English identifiers, per-TD traceability via backtick refs, horizontal rules between SIs.

````markdown
### SI-NN.X — {Verb} {Noun}

**Description:** One sentence — what this SI delivers and why it belongs here.

**Technical actions:**

1. Create `{module}/{file}.ts` — `{what it does}` (per `{slug}/TD-XX`)
2. Register `{Module}` in `{ParentModule}` — import + provider declaration
...
**Dependencies:** none _(or: SI-NN.1 — reason)_

**Acceptance criteria:**

- `POST /path` with `{valid payload}` returns `201` with `{key fields}`
- `POST /path` with `{invalid field}` returns `400` with `errorCode: "VALIDATION_ERROR"`
...
- **Xa-only pure-presentational screens** (Decision #33 conditional skip of Xb): `_(empty — pure presentational, smoke-gated by build/compile)_`
...
## Overflow policy (Decision #34) — detection + auto-split + FAILED fallback
