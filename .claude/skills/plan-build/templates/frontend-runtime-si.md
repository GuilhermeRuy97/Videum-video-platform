...
Read by Phase B step B4 when emitting SIs derived from a `### Frontend Runtime → ####` subsection. One TD subsection produces ≥1 plain SI: 1 Setup SI + N Migration SIs (one per Migration table row in the subsection) + 1 optional Verification SI (when Verification requires testing not already covered by Setup/Migration ACs).
...
**Plain `SI-NN.X` shape (no letter suffix, no dotted suffix).** Discriminator vs backend SIs is the subsection cited in Technical actions, not the naming. The conditional-skip rule for SI-Xb (Decision #33) does NOT apply — Frontend Runtime SIs are always plain SIs by construction.
...
## SI-NN.X' (Migration — ONE per Migration table row in the TD subsection)

````markdown
### SI-NN.X' — {Migration target file from Migration table} → {Pattern name}

**Frontend Runtime spec:** see `## Technical Specifications` → `### Frontend Runtime` → `#### {td-slug}/TD-NN — {topic}` → Migration row for `{file}`

**Technical actions:** (≤5 actions per file)

1. Read `{file}` → identify the current behavior pattern (per Migration row "Current behavior" cell).
2. Refactor `{file}` to apply the new pattern (per Migration row "Required change" cell). Field-name verbatim invariant (F2 protection) applies to any field/endpoint/lib name copied from the spec.
...
- `{file}` no longer contains the legacy pattern (per Migration row "Current behavior" — grep-verifiable).
- `{file}` uses the new pattern (per Migration row "Required change" — grep-verifiable).
...
## SI-NN.X'' (Verification — OPTIONAL; emit only when Verification requires test surface beyond Setup/Migration ACs)

````markdown
### SI-NN.X'' — {Pattern name} (Verification)

**Frontend Runtime spec:** see `## Technical Specifications` → `### Frontend Runtime` → `#### {td-slug}/TD-NN — {topic}` → Verification

**Technical actions:** (≤5 actions)

1. Add the integration / E2E tests listed in the Verification field that aren't already covered by individual Migration SI ACs.
2. {Optional 2nd action — add the regression guard tests when the Verification field calls them out explicitly}.
...
| Artifact | Layer | Test file |
|----------|-------|-----------|
| `{pattern-name}` (verification surface) | Integration / E2E per testing-guide-{subproject} — assertions enumerated in Verification field of the spec; this SI IS the test surface (Technical actions add the test files listed here) | `{test-file}` |
| `{regression-guard-target}` (when Verification calls one out) | Regression guard per testing-guide-{subproject} | `{test-file}` |

**Acceptance criteria:**

- Verification tests listed in the spec are present and passing.
...
- **One SI per Migration row** — never bundle multiple files into one Migration SI (overflow risk + atomicity loss).
- **Empty Migration table** (greenfield Setup-only adoption) → emit only the Setup SI; no Migration SIs; no Verification SI (covered by Setup ACs).
