...
- `/plan-rule-author "I need a plan-validate hard-fail rule that detects openapi.json with endpoints not mentioned in any inherited decisions doc"`
- `/plan-rule-author "build-time advisory: warn when a rendered section cites a file that was not listed in context.md's _Source files:_"`
- `/plan-rule-author "side-effect resolve: when a TD is decided with Option Z, sync the corresponding entry in library-refs.md"`
...
- **Dispatch point** — keywords `plan-validate` / `validate-time` / `validation` → `validate`; `plan-build` / `build-time` / `build-abort` / `sentinel` / `during build` → `build`; `plan-resolve` / `side-effect` / `edits decisions` / `applies revisions` → `resolve`.
- **Severity** — `hard-fail` / `aborts` → hard-fail; `advisory` / `warning` / `does not block` → advisory; `side-effect` → side-effect (forced for resolve).
...
From your description, I extracted:
  ...
  - Description: "openapi.json has endpoints not mentioned in any decisions doc"
...
- `[Accept]` → store the proposal as `NAME` and `PREFIX`; proceed to Step 3.
- `[Change name]` → enter override capture flow (below).
- `[Cancel]` → abort silently; nothing further is asked or written.

**Override capture flow.** When the user picks `[Change name]`, ask via inline assistant message (NOT via AskUserQuestion, which is choice-based):

```
Which name do you prefer? Provide it in kebab-case (e.g., openapi-consistency).
```
...
During this override capture loop, if the user types `cancel` (or clear negation) instead of a kebab-case name, abort silently.
...
Are these clear to a reader who has never seen this rule before? (yes / redo)
```

If user says `redo`, loop back to skeleton instructions and regenerate HUMAN_TITLE + OPENING_PARAGRAPH with adjusted prompt; re-show checkpoint. If `yes`, proceed to standard preview:
...
Write to disk? (yes / change <step-N> / cancel)
```

User responses:
- `yes` (or any clear affirmation) → proceed to Step 9.
- `change <step-N> {new value}` → loop back to that step, re-derive downstream sections, re-show preview.
- `cancel` (or any clear negation) → abort silently; nothing is written.
