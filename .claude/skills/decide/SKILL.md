---
name: decide
description: "Front-door for free-text decision needs. Triages user input against existing technical decisions, then routes to one of three branches: Revision (parameter change in existing decided TD), Supersede (Option letter change requires new TD), or Greenfield (no relevant TD exists). Trigger phrases: 'is there a decision about X?', 'I want to change Y', 'I need to decide Z', 'we already decided on...', 'the path/parameter/threshold of... changed', any free-text input describing a need that may or may not be covered by a TD. For structured phase research, use /research <phase> directly. For pipeline-stage operations on known scopes, use /plan-context, /plan-validate, /plan-resolve, /plan-build directly."
...
- `/decide "I want to move openapi.json from docs/ to the backend subproject"`
- `/decide "I need to adjust the rate limit threshold, it's too tight"`
- `/decide "is there a decision about auth token cache?"`
- `/decide "let's swap bcrypt for argon2"`
...
If the input is a phase reference (`phase NN`, `phase-NN-slug`, integer NN matching a phase) — meaning the user really wants structured phase research, not triage — abort with: `"Phase research has known shape. Use /research phase-NN directly to start a structured research cycle."`.

## Procedure

### Step 1 — Triage via `decisions-correlator`
...
### Step 2 — Disambiguation via `AskUserQuestion`
...
Always include a 4th option: `"None — create new TD"`.

The `AskUserQuestion` is single-select with these 4 options (3 candidates + "None").
...
- If `**Decision:**` is `_[pending]_` → abort with: `"TD-NN is pending — run /plan-validate <scope> to surface the OQ-N of the pending TD, then /plan-resolve <scope> to fill in the decision. /decide only operates on already-decided TDs: pending TDs do not support Revision (there is no decision to revise) nor Reaffirm (there is no decision to reaffirm). Skipping /plan-validate before /plan-resolve falls into 'validation.md is already clean — nothing to resolve' if the TD was never surfaced as an issue."`. Skill terminates without further questions.
- Otherwise (TD has a decided Option letter) → ask a follow-up question to classify the action:

```
"Which action to apply to {ref}?"
...
When the user picks `"None"`, jump to the Greenfield branch (Step 3).
...
- **Mechanism:**
  ...
  3. Issue a final `AskUserQuestion` with 2 options:
     - `[Accept and save]` — apply the Edit
     - `[Cancel — I'll edit manually]` — abort, no Edit
...
- **Mechanism:** terminate the skill (no edits) with a copy-paste-ready instruction:
  ...
  ```
  Next step: run /research <suggested-slug> to create the new replacement TD.

  Pass the following context to /research via the prompt:
    "This research will supersede <old-slug>/TD-NN — Option <X> (<name>).
     Consider the trade-off of the previous choice and the new need: <user input>."

  After /research completes (the new doc lives at docs/decisions/technical-decisions-<suggested-slug>.md
  with status: pending), run in order (using the slug of the NEW TD as the argument — not the old one,
  not a phase number):

      /plan-context <suggested-slug>     ← bootstrap task-mode (creates docs/tasks/task-<slug>/
                                          via mkdir -p; orphan ad-hoc with related_phases: []
                                          is accepted as task scope)
      /plan-validate <suggested-slug>    ← surface the new pending TD as an issue
      /plan-resolve <suggested-slug>     ← AskUserQuestion on the new pending TD; when the recommended
                                          option is chosen, plan-resolve reads the "supersedes <old-slug>/TD-NN"
                                          from the heading + Notes of the new TD and injects the marker
                                          <!-- status: superseded-by: <new-slug>/TD-NN --> in the old TD
                                          (cross-doc authority per plan-resolve/SKILL.md line 43).

  Do NOT recommend /plan-resolve directly (skipping context/validate). plan-resolve aborts with
  "validation.md not found" if run before plan-validate, which in turn aborts without context.md.
  ```

  - Substitute `<suggested-slug>` with a kebab-case slug derived from the user input (e.g., `auth-refresh-grace-period`, `openapi-spec-codegen`).
  - Substitute `<old-slug>/TD-NN`, `<X>`, `<nome>`, and `<user input>` with the actual values.
...
- **Mechanism:** mirror Supersede branch's abort pattern — terminate with explicit instruction:
  ...
  ```
  Next step: run /research <suggested-slug> to create the new greenfield TD.

  Your need: "<user input>"
  ```

  - `<suggested-slug>` derived from the user input.
...
- **E. Pipeline (run in order, as applicable):**
...
```
✓ Revision applied at <slug>/TD-NN (line NN-MM of docs/decisions/technical-decisions-<slug>.md).

Downstream — run manually as applicable:

A. Plan artifacts:
   - docs/tasks/task-<slug>/task-<slug>.md (3 stale references)
   - docs/tasks/task-<slug>/context.md (1 reference)
...
C. Tests:
   - no project test file references the old value.

D. Documentation:
   - <subproject>/<doc-file> (2 mentions)
   - <repo-doc>/<file>.md (1 mention)

E. Pipeline (run in order):
   1. /plan-context task-<slug>
   2. /plan-validate task-<slug>
   3. /plan-resolve task-<slug>        ← Libraries changed
   4. /plan-build task-<slug>          ← append-mode auto; pending SIs cite the parameter
```

Placeholders in the example above are filled at runtime with concrete values discovered by the skill (subproject directories from `affected_subprojects:` field of plan frontmatter, file extensions from each subproject's stack, etc.). When the skill actually emits the report, values are replaced with real paths — placeholders are template-only.

If a section has zero items, omit it entirely (no "Nothing to do" placeholders).

## Settled decisions (grill session 2026-04-27)

These decisions guided the skill design. They are not re-opened:

- **Triage zero matches** → top-3 shown even with low-confidence; user can pick one or create new.
- **Multiple matches** → AskUserQuestion single-select with "create new" escape; multi-select not supported.
- **Old docs migration** → forward-only; pre-2026-04-27 is opaque history.
- **Impact report section E** is dynamic; lists ONLY stages with a real trigger.
...
- **No free-form text input via AskUserQuestion.** Drafts are inferred and shown inline; user confirms via Accept/Cancel binary.
