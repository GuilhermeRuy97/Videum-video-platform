...
Violating this stop is the single most common failure mode of this skill. Treat "Proceed to SI-NN.X+1?" as a terminator, not a rhetorical question.
...
- After each SI (except the last) in default mode, **STOP after emitting "Proceed to SI-NN.X+1?"** and wait for user approval before any further tool call. Continuous mode applies only when the user explicitly requested it at the start of the session.
