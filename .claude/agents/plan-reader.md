...
1. **Locate phase headers.** Use `Grep -n '^## Phase ' docs/project-plan.md` to list every phase heading with its line number. If that pattern returns no matches, try the alternatives in order until one matches:

   - `^## Phase NN\b` with the integer `NN` literally interpolated (e.g., `^## Phase 02\b`) — catches non-standard separators between "Phase" and the number.
...
4. **Read the target phase range in full.** Bounded Read of the lines identified in step 2. This block is the primary output source — parse it for:
   ...
   - **Capabilities** — typically a bulleted list under a sub-heading like "Capabilities" or inline under the heading. Extract each bullet verbatim.
   - **Out of scope** — look for an `Out of scope:` line or bullet.
   - **Deliverables** — look for `Deliverables:` line.
   - **Affected subprojects** — look for explicit mentions of subproject paths (`nestjs-project`, `nextjs-project`, etc.) or a `Subprojects:` line.
   - **Sequencing / deferred pass notes** — any sentence about "first", "subsequent pass", "deferred" that affects scope.
