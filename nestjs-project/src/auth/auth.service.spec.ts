...
// `jest.Mocked<Service>` types method properties as class methods carrying `this`,
// which trips @typescript-eslint/unbound-method on `expect(mock.method)` — a known
// false positive with jest mocks. Accessing the method off a plain map of jest.Mock
// sidesteps it without loosening the rule for real production code.
type MockMap = Record<string, jest.Mock>;
const asMocks = (obj: unknown): MockMap => obj as MockMap;
